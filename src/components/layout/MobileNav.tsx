    ...
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent side="right" className="w-[320px] bg-background p-0">
        ...
        <nav className="mt-6 flex flex-col">
          {NAV_ITEMS.map((item) => (
            <div key={item.href}>
              <Link
                ...
                className={cn(
                  "block px-6 py-3 text-base font-medium transition-colors hover:bg-accent",
                  pathname === item.href
                    ? "border-l-2 border-primary pl-[22px] text-primary"
              ...
              {item.children?.map((child) => (
                <Link
                  ...
                  onClick={() => setOpen(false)}
                  className={cn(
                    "block px-10 py-2 text-sm transition-colors hover:bg-accent",
                    pathname === child.href
                      ? "text-primary"
                      : "text-muted-foreground"
                  )}
          ...
          <div className="flex flex-col gap-2 px-6 py-4 text-sm text-muted-foreground">
            <a
            ...
            <a
              ...
              className="flex items-center gap-2 transition-colors hover:text-primary"
            >
              <svg className="h-4 w-4 text-primary" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
              Facebook
            </a>
          </div>
        </nav>
